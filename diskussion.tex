...
TræningsMester står stærkest som en plausibel \emph{design- og implementeringsmodel}, hvor planlagt træning, faktisk logging, completion, feedback og næste handling adskilles. Det er relevant, fordi realistisk træningsadfærd sjældent følger en perfekt lineær plan. Designet og implementeringen giver fagligt belæg for fleksibel progression, mens effekt, drift og sikkerhed kræver senere undersøgelsestrin.

Fire spændinger afgør evidensens styrke: fleksibilitet mod datakvalitet, lav friktion mod detaljeret logging, app-evidens mod manglende effektstudie og sikker arkitektur mod ufuldstændig verifikation. Appens faglige værdi ligger i at gøre spændingerne håndterbare.
...
Sikkerhedsmodellen er stærkest som arkitektur og mere begrænset som verifikation. RLS, repositories og Edge Functions placerer adgangskontrol uden for klienten, mens rolle-, backend- og runtime-scenarier kræver mere test. Evidensgrundlaget er derfor tilstrækkeligt til en fagligt begrundet design- og implementeringscase. Effekt, fuld produktionsmodenhed og komplet sikkerhed hører til senere dokumentation.
...
Begrænsningen ligger i verifikationen. Tabel~\ref{tab:tm-test-traceability} viser dokumenterede build- og testspor samt åbne mangler i rolle-, backend- og runtime-scenarier. Belægget rækker derfor til et fagligt plausibelt og delvist verificeret sikkerhedsdesign. Sikkerhedscertificering, penetrationstest og komplet produktionsmodenhed kræver yderligere arbejde.

Sikkerhedsmodellen er derfor stærkest på designniveau. Den placerer adgangshåndhævelse og privilegeret logik server-side. Dækningsgraden er næste testtrin. Rolle- og tværbrugerscenarier skal testes, før designet kan omsættes til en stærkere driftspåstand.
...
Beta-feedbacken styrker vurderingen ved at dokumentere praktisk friktion omkring installation, søgning, katalogkvalitet, enheder, medier og watch. Tabel~\ref{tab:tm-beta-feedback} og Tabel~\ref{tab:tm-testflight-timeline} samler disse temaer. Grundtvig og Clotworthy understøtter, at kvalitative data kan give kontekstuel indsigt, men kræver tydelig afgrænsning \cite{grundtvig2025_analyse_data,clotworthy2025_etik_kvalitet_forskning}.
