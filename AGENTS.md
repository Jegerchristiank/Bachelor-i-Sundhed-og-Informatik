...
- `main.tex` indlæser `problemformulering_og_afgraensning.tex`, `teoretisk_og_faglig_ramme.tex`, `krav_og_use_cases.tex`, `systemarkitektur.tex`, `interaktionsdesign.tex` og `data_sikkerhed_og_interoperabilitet.tex` som kapitelinput, `kildesoegning.tex` og `billedbilag.tex` som bilag og bruger `referencer.bib` til referenceafsnittet.
